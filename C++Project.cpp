...
void player_menu2 ()
{
    cout << "Press 1 to view squad\n";
    cout << "Press 2 to see fixtures\n";
    cout << "Prees 3 to see top scorer\n";
    cout << "Press 4 to go back\n"; //go back to player_menu
}
void formation()
{
    cout << "Choose formation\n";
    cout << "1. 4-3-3\n";
    cout << "2. 4-4-2\n";
    cout << "3. 5-3-2\n";
}
...
class  Register {
    ...
    string logpass;
    public:
    void reg() //calling from 949
    {
        cout<<"Registration\nEnter user name : "; cin>>regname;
        this->regname = regname;
        cout<<"Enter password : "; cin>>regpass;
        this->regpass = regpass;
        ofstream reg("database.txt", ios:: app);//append mood ,if file is not created then it will create new file by that name .If created then it will continue from the end of
        reg << regname << ' ' << regpass << endl;// previously created file . writting to file
        cout<<"Registration successfull\n";


    }

    int login() //calling from 966
    {
        int exist;
        string u,p;
         ...
         while(input >> u >> p) //reding line by line from the database file till the end of file
         {
         ...
         if(exist == 1)
            {
                cout<<"Hello "<< logname << " Welcome to Football club Barcelona\n";
                return 1;
            }
        else
           { cout<<"Login error!!\nTry againn\n";
...
class Person :  public Register{
    ...
    virtual void getdata(int=0)=0; //derived class must have the defination of this function
...
class employee : public Person {
    ...
    void getdata(int t)     /////////   WRITE TO FILE
    {
        ...
        //writting to file
        fstream employee_information("employee_information.txt", ios::app); //If the file does not exist, it will be created.if created then it will continue from the end of previously created file
    ...
    void search()//calling from 810
    {

        int x=0;
        fstream d;
        d.open("employee_information.txt", ios::in); //in for reading
        string empID;
     ...
     void Delete() //calling from 790
     {
        ...
        ofstream e; //for writting
        e.open("new.txt");
        d >> ID >> name >> age >> nationality >> Job >> salary; //reading from d file
        while (!d.eof())
        {
            if (ID != deleteID)
            {
...
class player : public Person  {
     ...
     player(string name, string age, string nationality, string position, string jerseynumber,long double salary )
     {
        ...
        fstream player_information("Player_Information.txt", ios::app); //writting to file
     ...
     void getdata(int t) //calling from admin 951 , player 743
     {
        ...
        if (t==1)
        {
        cout<<"Set salary : "; cin>>salary; //only admin will set the salary
        }
        else
        {
            srand (time(NULL));
        ...
        fstream player_information("Player_Information.txt", ios::app);//writting to player_information file
        player_information << name << setw(25-name.size()) << ' ' << age << setw(10) << ' ' << nationality << setw(15-nationality.size()) << ' ' << position << setw(10-position.size())<<' ' << jerseynumber << setw(10-jerseynumber.size())<< ' '<< salary << endl;
     }
     void showdata()
     {
     ...
     void Delete() //calling from admin menu ,line 753
     {
        cout<<"Enter name to delete : ";
        string deletename;
        cin>> deletename;
        ifstream d; //for reading
        d.open("player_information.txt");
        ofstream e;
        e.open("new.txt");
        d >> name >> age >> nationality >> position >> jerseynumber >> salary; //Reading
        while (!d.eof())
        {
            if (name != deletename) //Wrtting to new file 'e'
            {
     ...
     void search() //calling from admin menu ,Line in 773
     {

        int x=0;
        fstream d;
        d.open("player_information.txt", ios::in); //for reading
        string jnum;
     ...
     {
        fstream d;
        d.open("Top_scorer.txt", ios::in);//for reading//written in line 832 .
        int maxgoal=0;
        int assi = 0;
        string top_scorer;
        while (!d.eof())
...
class squad : public player {
        ...
        int show_squad() //calling from line 976
        {
            formation();
            int x; cin >> x;
            int choice;
    ...
    ///////////////////////////////////////////////////////////
    void fixture() //called from line 984
    {
        ...
        int date = rand() % 27 + 1 ;
        int month = rand() % 12 + 1;
            cout <<setw(10)<< date << "/" << month << "/" << 2024 << "                                      " <<setw(20)<< "Barcelona Vs " << teams[match] << endl;
            i++;
        }
    }
...
int main()
{
   ...
   cout << "***************** WELCOME TO FOOTBALL CLUB BARCELONA *****************\n";
   Person *personptr[200];//arrey of 200 pointers pointing to objects of 'person' class
   int n=0;
   cout<<"Login as\n(1) Admin\n(2) Player\n(3) Exit\n";
   int x; cin >> x;
   ////////////////////////// LOG IN AS ADMIN //////////////
   ...
   if(x==1)
   {
    ...
    while(x!=0)
    {
        ...
        char ans;
        do {
        personptr[n] = new player; //'new' is used to dynamically allocate memory
                                     //which returns a pointer to allocated memory and pointing to derived class ( player )
        personptr[n]->getdata(1); //calling player class's (Derived class not base class's because of 'virtual') getdata function defination in line 226
        n++;
        ...
        {
          personptr[n] = new player; //pointing from player (derived) class
          personptr[n]->Delete();//deleting from player class ,Defination in line 254
          admin_menu();
        }
        if (choice==3)      /////// DISPLAY PLAYER class INFORMATION
        {
            ifstream player_information("player_information.txt"); //to read from file
            char ch;
            cout<<"Player"<<setw(22)<<"Age"<<setw(20)<<"Nationality"<<setw(10)<<"Position"<<setw(10)<<"Jersey"<<setw(10)<<"Salary"<<endl;
            while (player_information) //till end of file
            {
                player_information.get(ch); // get() is member function to read character by character from the file and it stores the character in variable 'ch'
                                             //after reading a character ,it points to next character
                cout<<ch;
            }
            cout<<endl;
        admin_menu();
        ...
        if (choice==4)  ///////     SEARCH PLAYER
        {
          ...
          personptr[n]->search(); //invoking player class's search function, Defination in line 279
          admin_menu();
        }
        ...
        char ans;
        do {
        personptr[n] = new employee;//pointing to employee class (derived )
        personptr[n]->getdata();//invoking employee class's (derived) getdata function
        n++;
        ...
        {
          personptr[n] = new employee;//pointing to employee class(derived)
          personptr[n]->Delete();//deleting from emplyee class,Defination in line 126
          admin_menu();
        }
         if (choice==7)     /////   DISPLAY EMPLOYEE INFORMATION
        {
            ...
            {
                employee_information.get(ch);//reading character by character and storing the character in variable 'ch'
                cout<<ch;
            }
            cout<<endl;
        admin_menu();
        ...
        if (choice==8)  //////////      SEARCH EMPLOYEE
        {
          ...
          personptr[n]->search();//invoking employee class's search function,Definmation in line 148
          admin_menu();
        }
        if (choice == 9)      //////    SQUAD INFORMATION
        {
          personptr[n] = new squad;//pointing  to derived class 'squad '
          personptr[n]->showdata();//invoking squad class's (derived) showdata() function .(function defination in line 569
            n++;
          n++;
        }
        ...
        if (choice == 10)       /////// SIMULATE MATCH
        {
            ...
            squad_report << "Player" <<setw(20) << "Goal" << setw(10) << "Assist" << endl;//writting to file
            ...
            fstream Top_scorer("Top_scorer.txt", ios::app);//writting to file(file used in line 303)
        ...
        {
            ifstream squad_report("squad_report.txt");//reading from squad_report file (line 832)
            char ch;//without using player class
            while (squad_report)
            {
                squad_report.get(ch);//reading character by character
                cout<<ch;
            }
            cout<<endl;
        ...
        {
            personptr[n] = new player;//pointing to player class (derived) .
            n++;
            personptr[n]->top_Scorer();//calling player class's top_scorer function .function defination in line 303
            n++;
            n++;
            admin_menu();
        }
        ...
        {
            personptr[n] = new squad;//pointing to squad class (derived)
            personptr[n] -> fixture();//calling squad class's (derived) fixture function .function defination in line 569
            n++;
            admin_menu();
        }
        ...
        if (choice == 0)    /////   EXIT
        {
          ...
          //  remove("squad_report.txt");
            ...
            fstream ofs;//ofs is object name of file
            ofs.open("Top_scorer.txt", ios::out | ios::trunc);//ios::out for writting and If the file does not exist, it will be created. If it already exists, its contents will be cleared.
            ofs.close();
   ...
   if (x==2)
   {
    ...
    b: //Levels are used to direct the flow of control along with goto statemennts ( Jump from line 998 ).
     player_menu();
     int choise;
     cin >> choise;
     while(choise != 50)
     {
        ...
        personptr[n] = new player; //pointing to player class (derived)
        personptr[n]->reg(); //multilevel inheritence = register class (reg () function)->person class->player class )
        cout<<"Please fill up the form\n";
        personptr[n]->getdata(0);//defination in line 226
        n++;
        cout << "Now login to see features\n";
        player_menu();//showing player menu
        }
        if(choise==2)       //////      LOG IN
         {
       ...
       int log = 0;
        ...
        {
           personptr[n] = new player;//pointing to player class (derived)
            int log = personptr[n]->login();//multilevel inheritence = register class (reg () function)->person class->player class )
            n++;
            if ( log == 1)  break;
        }
         a: //Levels are used to direct the flow of control along with goto statemennts ( Jump from line 986 ).
        player_menu2();
        int y; cin >> y;
        if ( y == 1 )           ////////    VIEW SQUAD
        {
            personptr[n] = new squad;//pointing to squad class (derived)
            int z = personptr[n] ->show_squad();//Defination in line 478 ( member funtion of squad class )
            n++;
            if ( z == 1 )   goto a;//press 1 to change the formation

        }
        if ( y == 2 )       ////////////    FIXTURE
        {
            personptr[n] = new squad;//pointing to squad class (derived )
            personptr[n] ->fixture();//calling squad class's fixture function .Line 569
            n++;
            goto a;//going to line 970 (payer menu )
        }
        if ( y == 3 )
        {

            personptr[n] = new player;
            personptr[n] ->top_Scorer();//defination in line 303 (player class)
            n++;
            goto a;//going to line 970 (player menu )
        }
        if ( y == 4)
        {
            goto b;//going to 939 (going back to 2nd main menu )
        }
        ...
        if ( choise == 3)
            {
                main();//press 3 to go to main menu
            }
